...
import fvmd.utils.basic
from fvmd.utils.basic import print_stats
import fvmd.utils.samp
...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8):
    ...
    for i in range(n_predictions):
        ...
        flow_loss += i_weight * fvmd.utils.basic.reduce_masked_mean(i_loss, valids)
...
class DeltaBlock(nn.Module):
    ...
    def forward(self, fcorr, flow):
        B, S, D = flow.shape
        assert(D==2)
        flow_sincos = fvmd.utils.misc.posemb_sincos_2d_xy(flow, self.latent_dim, cat_coords=True)
...
class Pips(nn.Module):
    ...
    def forward(self, trajs_e0, rgbs, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, feat_init=None, is_train=False, beautify=False):
        total_loss = torch.tensor(0.0, device=rgbs.device)  # vidaudit: device-agnostic (was .cuda())
        ...
        if feat_init is not None:
            ...
            feat1 = fvmd.utils.samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0], coords[:,0,:,1]).permute(0, 2, 1) # B,N,C
        ...
        for itr in range(iters):
            ...
            if itr >= 1:
                ...
                feats2_ = fvmd.utils.samp.bilinear_sample2d(fmaps2_, coords2_[:,:,0], coords2_[:,:,1]).permute(0, 2, 1) # B*S, N, C
                feats2 = feats2_.reshape(B,S,N,self.latent_dim)
                feats4_ = fvmd.utils.samp.bilinear_sample2d(fmaps4_, coords4_[:,:,0], coords4_[:,:,1]).permute(0, 2, 1) # B*S, N, C
